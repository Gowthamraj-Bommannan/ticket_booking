...
class RouteEdge(models.Model):
    ...
    from_station = models.ForeignKey(
        Station, on_delete=models.CASCADE, related_name="edges_from"
    )
    to_station = models.ForeignKey(
        Station, on_delete=models.CASCADE, related_name="edges_to"
    )
...
class RouteTemplate(models.Model):
    ...
    CATEGORY_CHOICES = [("local", "local"), ("fast", "fast")]

    name = models.CharField(max_length=100)
    from_station = models.ForeignKey(
        Station, related_name="template_starts", on_delete=models.CASCADE
    )
    to_station = models.ForeignKey(
        Station, related_name="template_ends", on_delete=models.CASCADE
    )
