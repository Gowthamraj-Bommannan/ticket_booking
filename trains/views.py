...
class TrainViewSet(viewsets.ModelViewSet):
        ...
        try:
            ...
            if train_number:
                inactive_train = Train.all_objects.filter(
                    train_number=train_number, is_active=False).first()
                if inactive_train:
    ...
    def update(self, request, *args, **kwargs):
        ...
        logger.info(f"Attempting to update train {instance.train_number} with data: {request.data}")
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        instance.save()
        logger.info(f"Train soft-deleted (is_active set to False): {instance.train_number}")
        return Response({'detail': f'Train {instance.name} ({instance.train_number}) has been removed.'}, 
...
class TrainScheduleViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        is_fast = hasattr(route_template, 'category') and getattr(route_template, 'category', '').lower() == 'fast'
        distances = []
        if is_fast:
        ...
        stops_with_time = self._generate_schedule_timings(stations, distances, start_time)
        ...
        for sched in existing_schedules:
            ...
            if (new_start < sched_end and sched_start < new_end):
                ...
                raise ScheduleAlreadyExists(
                    TrainMessage.TRAIN_SCHEDULE_OVERLAPS
                )
    ...
    def _find_shortest_path(self, code_a, code_b):
        ...
        graph = {}
        for edge in edges:
            graph.setdefault(edge.from_station.code.upper(), []).append((edge.to_station.code.upper(), edge.distance))
            if edge.is_bidirectional:
                graph.setdefault(edge.to_station.code.upper(), []).append((edge.from_station.code.upper(), edge.distance))
        # Dijkstra
    ...
    @staticmethod
    def _generate_schedule_timings(stations, distances, start_time, speed=35, halt_min=1):
        """
